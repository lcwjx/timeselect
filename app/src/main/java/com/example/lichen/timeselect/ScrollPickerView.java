...
public class ScrollPickerView extends View {
    ...
    private int mMinTextSize = 20; // 最小的字体
    private int mMaxTextSize = 24; // 最大的字体
    // 字体渐变颜色
    ...
    private void drawItem(Canvas canvas, int position, int relative) {
        ...
        if (relative == -1) { // 上一个
            ...
            if (mMoveLength < 0) { // 向上滑动
                ...
                mPaint.setTextSize((mMinTextSize - (mMaxTextSize - mMinTextSize)) + (mMaxTextSize - mMinTextSize)
                        * mMoveLength / mItemHeight);
            }
            ...
            mPaint.setTextSize(mMinTextSize - ((mMaxTextSize - mMinTextSize) * 2));
        }
        ...
        drawCenterLine(canvas, relative);
     ...
     * @param relative
     */
    private void drawCenterText(Canvas canvas, int relative) {
          if (relative==0) {
              ...
              if (!TextUtils.isEmpty(unit)) {
                  float measureText = paint.measureText(unit)*3.8f;
                  paint.setTextSize(mMaxTextSize);
                  canvas.drawText(unit,mMeasureWidth-measureText,y,paint);
              }
          }
    }
     ...
     * @param relative
     */
    private void drawCenterLine(Canvas canvas, int relative) {
        if (relative == 0) {
            Paint paint = new Paint();
            paint.setColor(getResources().getColor(R.color.colorPrimaryDark));
            paint.setStrokeWidth((float) 1);
            canvas.drawLine(0, mItemHeight * 2, mMeasureWidth, mItemHeight * 2, paint);
            canvas.drawLine(0, mItemHeight * 3, mMeasureWidth, mItemHeight * 3, paint);
        }
    }
